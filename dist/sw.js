(() => {
  ...
  function tempTransaction(db3, mode, storeNames, fn) {
    if (!db3.idbdb || !db3._state.openComplete && (!PSD.letThrough && !db3._vip)) {
      ...
      if (!db3._state.isBeingOpened) {
        if (!db3._options.autoOpen)
          return rejection(new exceptions.DatabaseClosed());
        db3.open().catch(nop);
      }
      return db3._state.dbReadyPromise.then(() => tempTransaction(db3, mode, storeNames, fn));
    } else {
      ...
      try {
        ...
        if (ex.name === errnames.InvalidState && db3.isOpen() && --db3._state.PR1398_maxLoop > 0) {
          ...
          db3._close();
          return db3.open().then(() => tempTransaction(db3, mode, storeNames, fn));
        }
        return rejection(ex);
      }
  ...
  function createWhereClauseConstructor(db3) {
    return makeClassConstructor(WhereClause.prototype, function WhereClause2(table, index, orCollection) {
      ...
      const indexedDB2 = db3._deps.indexedDB;
      if (!indexedDB2)
  ...
  function createDBCore(db3, IdbKeyRange, tmpTrans) {
    function extractSchema(db4, trans) {
      const tables2 = arrayify(db4.objectStoreNames);
      return {
        schema: {
          name: db4.name,
    ...
    const { schema, hasGetAll } = extractSchema(db3, tmpTrans);
  ...
  function createMiddlewareStack(stackImpl, middlewares) {
    return middlewares.reduce((down, { create }) => __spreadValues(__spreadValues({}, down), create(down)), stackImpl);
  }
  ...
  function generateMiddlewareStacks({ _novip: db3 }, tmpTrans) {
    const idbdb = tmpTrans.db;
    const stacks = createMiddlewareStacks(db3._middlewares, idbdb, db3._deps, tmpTrans);
    db3.core = stacks.dbcore;
    db3.tables.forEach((table) => {
      const tableName = table.name;
      if (db3.core.schema.tables.some((tbl) => tbl.name === tableName)) {
  ...
  function setApiOnPlace({ _novip: db3 }, objs, tableNames, dbschema) {
  ...
  function removeTablesApi({ _novip: db3 }, objs) {
    objs.forEach((obj) => {
      for (let key in obj) {
  ...
  function runUpgraders(db3, oldVersion, idbUpgradeTrans, reject) {
    const globalSchema = db3._dbSchema;
    const trans = db3._createTransaction("readwrite", db3._storeNames, globalSchema);
    ...
    newScope(() => {
      ...
      if (oldVersion === 0) {
        ...
        DexiePromise.follow(() => db3.on.populate.fire(trans)).catch(rejectTransaction);
      } else
        updateTablesAndIndexes(db3, oldVersion, trans, idbUpgradeTrans).catch(rejectTransaction);
    });
  }
  function updateTablesAndIndexes({ _novip: db3 }, oldVersion, trans, idbUpgradeTrans) {
    const queue = [];
    const versions = db3._versions;
    let globalSchema = db3._dbSchema = buildGlobalSchema(db3, db3.idbdb, idbUpgradeTrans);
    ...
    versToRun.forEach((version) => {
      queue.push(() => {
        ...
        if (contentUpgrade && version._cfg.version > oldVersion) {
          ...
          setApiOnPlace(db3, [db3.Transaction.prototype], keys(upgradeSchema), upgradeSchema);
  ...
  function readGlobalSchema({ _novip: db3 }, idbdb, tmpTrans) {
    db3.verno = idbdb.version / 10;
    const globalSchema = db3._dbSchema = buildGlobalSchema(db3, idbdb, tmpTrans);
  ...
  function verifyInstalledSchema(db3, tmpTrans) {
    const installedSchema = buildGlobalSchema(db3, db3.idbdb, tmpTrans);
    const diff = getSchemaDiff(installedSchema, db3._dbSchema);
    return !(diff.add.length || diff.change.some((ch) => ch.add.length || ch.change.length));
  }
  function adjustToExistingIndexNames({ _novip: db3 }, schema, idbtrans) {
  ...
  function dexieOpen(db3) {
    const state = db3._state;
    const { indexedDB: indexedDB2 } = db3._deps;
    if (state.isBeingOpened || db3.idbdb)
      return state.dbReadyPromise.then(() => state.dbOpenError ? rejection(state.dbOpenError) : db3);
    ...
    return DexiePromise.race([openCanceller, (typeof navigator === "undefined" ? DexiePromise.resolve() : idbReady()).then(() => new DexiePromise((resolve, reject) => {
      ...
      if (!indexedDB2)
        throw new exceptions.MissingAPI();
      const dbName = db3.name;
      const req = state.autoSchema ? indexedDB2.open(dbName) : indexedDB2.open(dbName, Math.round(db3.verno * 10));
      if (!req)
        throw new exceptions.MissingAPI();
      req.onerror = eventRejectHandler(reject);
      ...
      req.onupgradeneeded = wrap((e) => {
        ...
        if (state.autoSchema && !db3._options.allowEmptyDB) {
          ...
          wasCreated = oldVer < 1;
          db3._novip.idbdb = req.result;
          runUpgraders(db3, oldVer / 10, upgradeTransaction, reject);
        }
      }, reject);
      req.onsuccess = wrap(() => {
        upgradeTransaction = null;
        const idbdb = db3._novip.idbdb = req.result;
        ...
        if (objectStoreNames.length > 0)
          try {
            ...
            if (state.autoSchema)
              ...
              if (!verifyInstalledSchema(db3, tmpTrans)) {
      ...
      return DexiePromise.resolve(vip(() => db3.on.ready.fire(db3.vip))).then(function fireRemainders() {
        if (state.onReadyBeingFired.length > 0) {
          let remainders = state.onReadyBeingFired.reduce(promisableChain, nop);
          state.onReadyBeingFired = [];
          return DexiePromise.resolve(vip(() => remainders(db3.vip))).then(fireRemainders);
        }
      });
    }).finally(() => {
  ...
  function enterTransactionScope(db3, mode, storeNames, parentTransaction, scopeFunc) {
    return DexiePromise.resolve().then(() => {
      const transless = PSD.transless || PSD;
      const trans = db3._createTransaction(mode, storeNames, db3._dbSchema, parentTransaction);
      const zoneProps = {
        trans,
        transless
      };
      if (parentTransaction) {
        ...
        try {
          ...
          if (ex.name === errnames.InvalidState && db3.isOpen() && --db3._state.PR1398_maxLoop > 0) {
            console.warn("Dexie: Need to reopen db");
            db3._close();
            return db3.open().then(() => enterTransactionScope(db3, mode, storeNames, null, scopeFunc));
          }
          return rejection(ex);
        }
      }
  ...
  function createVirtualIndexMiddleware(down) {
    return __spreadProps(__spreadValues({}, down), {
      table(tableName) {
        ...
        function addVirtualIndexes(keyPath, keyTail, lowLevelIndex) {
          ...
          const virtualIndex = __spreadProps(__spreadValues({}, lowLevelIndex), {
            isVirtual,
            keyTail,
            keyLength,
            extractKey: getKeyExtractor(keyPath),
        ...
        const result = __spreadProps(__spreadValues({}, table), {
  ...
  var hooksMiddleware = {
    ...
    level: 2,
    create: (downCore) => __spreadProps(__spreadValues({}, downCore), {
      table(tableName) {
        const downTable = downCore.table(tableName);
        const { primaryKey } = downTable.schema;
        const tableMiddleware = __spreadProps(__spreadValues({}, downTable), {
          mutate(req) {
            ...
            function addPutOrDelete(req2) {
              ...
              req2 = req2.type === "add" || req2.type === "put" ? __spreadProps(__spreadValues({}, req2), { keys: keys2 }) : __spreadValues({}, req2);
              if (req2.type !== "delete")
              ...
              return downTable.query({ trans, values: false, query: { index: primaryKey, range }, limit }).then(({ result }) => {
                return addPutOrDelete({ type: "delete", keys: result, trans }).then((res) => {
                  ...
                  if (result.length < limit) {
                    ...
                    return deleteNextChunk(trans, __spreadProps(__spreadValues({}, range), { lower: result[result.length - 1], lowerOpen: true }), limit);
                  }
                });
              });
            }
  ...
  function rebalance(target) {
    ...
    if (r) {
      const l = r === "r" ? "l" : "r";
      const rootClone = __spreadValues({}, target);
    ...
    create: (core) => {
      ...
      return __spreadProps(__spreadValues({}, core), {
        table: (tableName) => {
          ...
          const tableClone = __spreadProps(__spreadValues({}, table), {
            mutate: (req) => {
              ...
              if (subscr) {
                ...
                if (!queriedIndex.isPrimaryKey) {
                  if (method === "count") {
                    ...
                    const keysPromise = method === "query" && outbound && req.values && table.query(__spreadProps(__spreadValues({}, req), {
                      values: false
                    }));
  ...
  var Dexie$1 = class {
    constructor(name, options) {
      ...
      this._options = options = __spreadValues({
            ...
            if (state2.openComplete) {
              ...
              if (!bSticky)
                subscribe(function unsubscribe() {
                  db3.on.ready.unsubscribe(subscriber);
                  db3.on.ready.unsubscribe(unsubscribe);
                });
            }
          });
        };
  ...
  var Dexie = Dexie$1;
  props(Dexie, __spreadProps(__spreadValues({}, fullNameExceptions), {
    delete(databaseName) {
      const db3 = new Dexie(databaseName, { addons: [] });
      return db3.delete();
    },
    exists(name) {
      return new Dexie(name, { addons: [] }).open().then((db3) => {
        db3.close();
        return true;
  ...
  // src/config/conection.js
  var db = new Dexie$1("scrapping");
  db.version(1).stores({
    profiles: "++id, contactInfo, experience"
  });
  var db2 = new Dexie$1("profilesUrls");
  ...
  var openDB = indexedDB.open("candidates", 1);
})();
